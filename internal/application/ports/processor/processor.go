...
type Input struct {
	...
	// расширений; docs/METADATA_STORE.md, раздел 8.3).
...
// Реализуется libvips-бэкендом (код вынесен из applyDetectionCrop).
// Отсутствие поддержки ⇒ деградация (запрос с fc/oc обрабатывается по
// старой схеме: self-detection внутри процессора).
