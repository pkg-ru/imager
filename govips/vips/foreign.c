...
int set_heifsave_options(VipsOperation *operation, SaveParams *params) {
  // NB: "strip" is a common VipsForeignSave argument (deprecated alias of
  // "keep") — setting it prevents libvips from synthesizing fresh EXIF
  // (vips__exif_update) from the image header at save time.
...
int set_jxlsave_options(VipsOperation *operation, SaveParams *params) {
  ...
  // (vips__exif_update) from the image header at save time.
  int ret = vips_object_set(
      VIPS_OBJECT(operation), "strip", params->stripMetadata,
      "tier", params->jxlTier,
