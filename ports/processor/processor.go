...
// ErrTooManyConcurrency — сигнал перегрузки процессора: bounded очередь
// ожидания слота переполнена (или истёк бюджет ожидания). Возвращается
// адаптерами (libvips) при превышении лимита ожидающих запросов.
// Sentinel живёт в ports, чтобы app-слой мог распознавать перегрузку через
// errors.Is без зависимости от конкретных адаптеров (тот же паттерн, что
// bounded.ErrOutputLimitExceeded).
var ErrTooManyConcurrency = errors.New("processor: too many concurrent requests waiting for a slot")

// ErrTooManyDetectionConcurrency — сигнал перегрузки detection-семафора
// (ONNX-инференс): аналог ErrTooManyConcurrency для отдельного слота
// детекции. Возвращается при переполнении очереди ожидания или истечении
// max-wait.
var ErrTooManyDetectionConcurrency = errors.New("processor: too many concurrent requests waiting for a detection slot")

// IsTooManyConcurrency сообщает, является ли err сигналом перегрузки
// процессора (основной или detection-семафор). Распознавание по identity
// (errors.Is), а не по тексту: переименование сообщения не ломает маппинг.
func IsTooManyConcurrency(err error) bool {
	if err == nil {
		return false
	}
	return errors.Is(err, ErrTooManyConcurrency) || errors.Is(err, ErrTooManyDetectionConcurrency)
}
