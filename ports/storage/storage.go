...
// PrefixDeleter — ОПЦИОНАЛЬНЫЙ интерфейс пакетного удаления всех ключей с
// данным префиксом одним каталогом/пакетом (например, для admin DELETE по
// исходнику). Не является частью обязательного контракта ResultStore:
// адаптеры без DeleteByPrefix не поддерживают пакетное удаление.
//
// Семантика:
//   - идемпотентно: если по префиксу ничего нет — возвращается (0, nil);
//   - префикс сравнивается С граничным '/' (см. реализацию): префикс "dir/"
//     НЕ совпадает с ключами "dir2/...";
//   - возвращает число удалённых ключей.
type PrefixDeleter interface {
	// DeleteByPrefix удаляет все ключи, начинающиеся с prefix, и возвращает
	// число удалённых ключей.
	DeleteByPrefix(ctx context.Context, prefix object.ObjectKey) (int64, error)
}
