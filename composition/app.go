...
type AppOptions struct {
	...
	// воркерами после ответа клиенту; false = публикация синхронная (кэш
	// готов сразу после Generate) — используется в тестах, которые проверяют
	// состояние кэша сразу после запроса.
	AsyncPublish bool
}
...
func Build(ctx context.Context, opt AppOptions) (*App, error) {
	...
	// Координатор. WaitTimeout — страховка от "зависшего" владельца
	// генерации (зависший IO в процессоре, баг): waiter получает
	// ErrWaitTimeout вместо вечного ожидания. 0 = отключено (вечное
	// ожидание до ctx waiter'а). Дефолт 60s (2×DefaultGenerateTimeout).
	coord := singleflight.New(singleflight.Options{
		WaitTimeout: opt.SingleflightWaitTimeout,
	})
	...
	svc, err := generatev2.New(generatev2.Deps{
		...
		ResizeBackground:         compiled.DefaultResizeBackground,
		Logger:                   opt.HTTP.Logger,
