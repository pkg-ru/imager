...
// TestEffectiveAdmissionLimitFallback проверяет fallback-логику:
// application.limits.concurrency применяется как admission-лимит (со
// множителем) только когда http.max-concurrent-requests не задан; явный
// http.max-concurrent-requests всегда приоритетен; если оба не заданы —
// admission выключен (0).
func TestEffectiveAdmissionLimitFallback(t *testing.T) {
	t.Run("explicit max concurrent takes priority", func(t *testing.T) {
		// Явный http.max-concurrent-requests приоритетен даже при заданном
		// limits.concurrency: возвращается как есть, без множителя.
		if got := EffectiveAdmissionLimit(10, 4); got != 10 {
			t.Fatalf("EffectiveAdmissionLimit(10, 4) = %d, want 10", got)
		}
	})

	t.Run("limits concurrency fallback", func(t *testing.T) {
		// limits.concurrency задан, max-concurrent не задан (0) → применяется
		// limits.concurrency × AdmissionMaxConcurrentMultiplier.
		want := 4 * AdmissionMaxConcurrentMultiplier
		if got := EffectiveAdmissionLimit(0, 4); got != want {
			t.Fatalf("EffectiveAdmissionLimit(0, 4) = %d, want %d", got, want)
		}
	})

	t.Run("both zero means disabled", func(t *testing.T) {
		// Оба не заданы → admission выключен (0), текущее поведение.
		if got := EffectiveAdmissionLimit(0, 0); got != 0 {
			t.Fatalf("EffectiveAdmissionLimit(0, 0) = %d, want 0", got)
		}
	})

	t.Run("concurrency zero with explicit max", func(t *testing.T) {
		// max-concurrent задан, concurrency = 0 → приоритет max-concurrent.
		if got := EffectiveAdmissionLimit(8, 0); got != 8 {
			t.Fatalf("EffectiveAdmissionLimit(8, 0) = %d, want 8", got)
		}
	})
}
...
// TestParseRuntimeConfigSingleflightWaitTimeout проверяет декодирование
// application.singleflight-wait-timeout: явное значение, дефолт 60s и
// отключение (0).
func TestParseRuntimeConfigSingleflightWaitTimeout(t *testing.T) {
	// Явное значение.
	rc, err := ParseRuntimeConfig([]byte(`
version: "1"
policy: {}
application:
  singleflight-wait-timeout: "90s"
`))
	if err != nil {
		t.Fatalf("ParseRuntimeConfig: %v", err)
	}
	if rc.SingleflightWaitTimeout != 90*time.Second {
		t.Errorf("SingleflightWaitTimeout = %s, want 90s", rc.SingleflightWaitTimeout)
	}

	// Дефолт: не задано → 60s (2×DefaultGenerateTimeout).
	rc, err = ParseRuntimeConfig([]byte(`
version: "1"
policy: {}
`))
	...
	if rc.SingleflightWaitTimeout != 60*time.Second {
		t.Errorf("SingleflightWaitTimeout default = %s, want 60s", rc.SingleflightWaitTimeout)
	}

	// 0 = отключено (вечное ожидание до ctx waiter'а).
	rc, err = ParseRuntimeConfig([]byte(`
version: "1"
policy: {}
application:
  singleflight-wait-timeout: "0s"
`))
	if err != nil {
		t.Fatalf("ParseRuntimeConfig(0): %v", err)
	}
	if rc.SingleflightWaitTimeout != 0 {
		t.Errorf("SingleflightWaitTimeout = %s, want 0 (disabled)", rc.SingleflightWaitTimeout)
	}
}

// TestParseRuntimeConfigServerMetricsAuth проверяет декодирование
// server.metrics-auth: токен и allowed-ips пробрасываются в
// ServerConfig.MetricsAuth; не заданная секция — пустая (без авторизации).
func TestParseRuntimeConfigServerMetricsAuth(t *testing.T) {
	rc, err := ParseRuntimeConfig([]byte(`
version: "1"
policy: {}
server:
...
`))
	...
	if rc.Server.MetricsAuth.Token != "secret-token" {
		t.Errorf("MetricsAuth.Token = %q, want secret-token", rc.Server.MetricsAuth.Token)
	}
	if len(rc.Server.MetricsAuth.AllowedIPs) != 2 {
		t.Errorf("MetricsAuth.AllowedIPs len = %d, want 2", len(rc.Server.MetricsAuth.AllowedIPs))
	}
	if rc.Server.MetricsAuth.AllowedIPs[0] != "127.0.0.1" {
		t.Errorf("MetricsAuth.AllowedIPs[0] = %q, want 127.0.0.1", rc.Server.MetricsAuth.AllowedIPs[0])
	}
	if rc.Server.MetricsAuth.AllowedIPs[1] != "10.0.0.0/8" {
		t.Errorf("MetricsAuth.AllowedIPs[1] = %q, want 10.0.0.0/8", rc.Server.MetricsAuth.AllowedIPs[1])
	}

	// Не задано → пустая конфигурация (без авторизации, как сейчас).
	rc, err = ParseRuntimeConfig([]byte(`
version: "1"
policy: {}
`))
	...
	if rc.Server.MetricsAuth.Token != "" {
		t.Errorf("MetricsAuth.Token default = %q, want empty", rc.Server.MetricsAuth.Token)
	}
	if len(rc.Server.MetricsAuth.AllowedIPs) != 0 {
		t.Errorf("MetricsAuth.AllowedIPs default len = %d, want 0", len(rc.Server.MetricsAuth.AllowedIPs))
	}
}

// TestParseRuntimeConfigServerWriteTimeout проверяет декодирование
// server.write-timeout: явное значение сохраняется, не заданное — 0
// (runtime применит дефолт GenerateTimeout + запас).
func TestParseRuntimeConfigServerWriteTimeout(t *testing.T) {
	// Явное значение.
	rc, err := ParseRuntimeConfig([]byte(`
version: "1"
policy: {}
...
`))
	...
	if rc.Server.WriteTimeout != 120*time.Second {
		t.Errorf("Server.WriteTimeout = %s, want 120s", rc.Server.WriteTimeout)
	}

	// Не задано → 0 (runtime применит дефолт).
	rc, err = ParseRuntimeConfig([]byte(`
version: "1"
policy: {}
`))
	...
	if rc.Server.WriteTimeout != 0 {
		t.Errorf("Server.WriteTimeout default = %s, want 0 (runtime default)", rc.Server.WriteTimeout)
	}
}

// TestParseRuntimeConfigSingleflightWaitTimeoutInvalid — fail-fast: отрицательное
// значение или неверный duration — ошибка старта.
func TestParseRuntimeConfigSingleflightWaitTimeoutInvalid(t *testing.T) {
	cases := []struct {
		name string
		yaml string
	}{
...
policy: {}
application:
  singleflight-wait-timeout: "soon"
`,
		},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			_, err := ParseRuntimeConfig([]byte(c.yaml))
			if err == nil {
