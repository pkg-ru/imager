...
// Дешёвый lookup в map под g.mu (без аллокаций). Используется admission
// control для bypass: запрос, который может присоединиться к уже идущей
// singleflight-генерации того же ассета, пропускается в обход семафора.
func (g *Group) InFlight(key object.ObjectKey) bool {
	g.mu.Lock()
	_, ok := g.inflight[string(key)]
	g.mu.Unlock()
	return ok
}
