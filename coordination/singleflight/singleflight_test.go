...
// TestInFlightTrueDuringGenerationFalseAfter проверяет, что InFlight
// возвращает true, пока для ключа идёт операция, и false после её
// завершения (включая удаление из map).
func TestInFlightTrueDuringGenerationFalseAfter(t *testing.T) {
	g := New(Options{})
	key := object.ObjectKey("k")
	...
	// Пока владелец выполняется — InFlight == true.
	<-started
	if !g.InFlight(key) {
		t.Fatal("InFlight = false during generation, want true")
	}
	// Другой ключ — false.
	if g.InFlight(object.ObjectKey("other")) {
		t.Fatal("InFlight(other) = true, want false")
	}

	// После завершения владельца — InFlight == false.
	close(release)
	<-done
	if g.InFlight(key) {
		t.Fatal("InFlight = true after generation, want false")
	}
}
