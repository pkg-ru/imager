...
// семофор). Sentinel живёт в ports/processor (processor.ErrTooManyConcurrency),
// чтобы app-слой распознавал перегрузку через errors.Is без зависимости от
// адаптера. Здесь — алиас для обратной совместимости (используется в
// shared.NewSemaphore и тестах).
var ErrTooManyConcurrency = processor.ErrTooManyConcurrency
...
func (p *Processor) Process(ctx context.Context, in processor.Input, out io.Writer) (*processor.Result, error) {
	...
	// освобождается (defer p.sem.release() выше). Ёмкость семафора
	// (concurrency) передаётся в watchdog для наблюдаемости сиротских
	// операций: при превышении числа активных сирот над ёмкостью
	// логируется warning (реальная загрузка VIPS выше учтённой).
	br, err := runWatchdog(runCtx, p.limits.Concurrency, func() (*backendResult, error) {
...
//
// Наблюдаемость сиротских операций (A5): если основная горутина ушла по
// ctx.Done(), а cgo-операция продолжает выполняться, она помечается
// сиротой (orphan). При этом:
//   - инкрементируется счётчик imager_vips_orphan_ops_total (глобально,
//     т.к. у watchdog нет доступа к экземпляру Metrics);
//   - инкрементируется gauge imager_vips_orphan_inflight (текущее число
//     активных сирот);
//   - при завершении сироты gauge декрементируется, и если число активных
//     сирот превышает ёмкость семафора (concurrency) — логируется warning:
//     реальная загрузка VIPS выше учтённой, слоты «мнимо свободны».
//
...
func runWatchdog[T any](ctx context.Context, concurrency int, fn func() (T, error)) (T, error) {
	done := make(chan watchdogResult[T], 1)
	var orphan atomic.Bool
	go func() {
		out, err := fn()
		// Сирота: основная горутина уже ушла по ctx, результат игнорируется.
		// Декрементируем gauge и проверяем превышение ёмкости семафора.
		if orphan.Load() {
			orphanDone(concurrency)
		}
	...
	case r := <-done:
		return r.out, r.err
	case <-ctx.Done():
		// Операция продолжает выполняться в cgo — фиксируем сироту.
		orphan.Store(true)
		observability.IncVipsOrphanOpGlobal()
		observability.SetVipsOrphanInflightGlobal(orphanInflight() + 1)
		var zero T
		return zero, ctx.Err()
	}
}

// orphanInflight возвращает текущее число активных сиротских операций
// (глобальный gauge imager_vips_orphan_inflight). Используется для
// атомарного инкремента: expvar.Int.Add отсутствует в публичном API,
// поэтому читаем текущее значение и инкрементируем через Set.
func orphanInflight() int64 {
	return observability.VipsOrphanInflightGlobal()
}

// orphanDone вызывается при завершении сиротской cgo-операции: декрементирует
// gauge активных сирот и логирует warning, если число активных сирот
// превышает ёмкость семафора (concurrency) — реальная загрузка VIPS выше
// учтённой, слоты «мнимо свободны». Проверка дешёвая и выполняется только
// при завершении сироты, не на каждый вызов.
func orphanDone(concurrency int) {
	cur := orphanInflight()
	if cur > 1 {
		observability.SetVipsOrphanInflightGlobal(cur - 1)
	} else {
		observability.SetVipsOrphanInflightGlobal(0)
	}
	if cur > int64(concurrency) {
		slog.Default().Warn("libvips: orphan cgo operations in flight exceed semaphore capacity",
			"orphan_inflight", cur, "semaphore_capacity", concurrency)
	}
}
