...
// TestIsDetectionOverload — предикат деградации распознаёт ТОЛЬКО ошибки
// перегрузки detection-семафора (ErrTooManyDetectionConcurrency), а не
// реальные ошибки инференса/контекста.
func TestIsDetectionOverload(t *testing.T) {
	if !isDetectionOverload(ErrTooManyDetectionConcurrency) {
		t.Fatal("isDetectionOverload(ErrTooManyDetectionConcurrency) = false, want true")
	}
	// Оборачивание не теряет распознавание.
	if !isDetectionOverload(fmt.Errorf("libvips: face-crop: detection semaphore: %w", ErrTooManyDetectionConcurrency)) {
		t.Fatal("wrapped overload error must be recognized")
	}
	// Ошибки модели/контекста НЕ являются перегрузкой.
	if isDetectionOverload(errors.New("onnx: inference failed")) {
		t.Fatal("inference error must not be treated as overload")
	}
	if isDetectionOverload(context.Canceled) {
		t.Fatal("context.Canceled must not be treated as overload")
	}
	if isDetectionOverload(nil) {
		t.Fatal("nil must not be treated as overload")
	}
	// Видимая перегрузка основного семафора (другой sentinel) — не detection.
	if isDetectionOverload(ErrTooManyConcurrency) {
		t.Fatal("ErrTooManyConcurrency must not be treated as detection overload")
	}
}

// TestGateOverloadKeepsVipsSlotUsable — после перегрузки detection-слота
// (деградация center-crop) vips-слот остаётся у вызывающего и доступен для
// ПРОДОЛЖЕНИЯ обработки (кроп/ресайз/экспорт) без reacquire и без утечки.
func TestGateOverloadKeepsVipsSlotUsable(t *testing.T) {
	g := newTestGate(1, 1, 50*time.Millisecond)
	slot, err := g.acquireVips(context.Background())
	if err != nil {
		t.Fatalf("acquireVips: %v", err)
	...
	// Занимаем единственный detection-слот «чужим» запросом — handoff
	// гарантированно откажется по maxWait (перегрузка).
	if err := g.det.Acquire(context.Background()); err != nil {
		t.Fatalf("blocker det.Acquire: %v", err)
	}
	err = slot.handoffToDetection(context.Background())
	...
	if !isDetectionOverload(err) {
		t.Fatalf("err = %v, want detection overload", err)
	}

	// Деградация: vips-слот НЕ освобождался и НЕ перекладывался — состояние
	// консистентно для продолжения обработки (фаза кропа/ресайза).
	if !slot.vipsHeld || slot.detHeld {
		t.Fatalf("after overload: vipsHeld=%v detHeld=%v, want true/false", slot.vipsHeld, slot.detHeld)
	}
	// «Продолжение обработки»: слот можно использовать и корректно
	// освободить; двойной Release не должен ничего сломать.
	slot.Release()
	slot.Release()
	if slot.vipsHeld || slot.detHeld {
		t.Fatalf("release after overload: vipsHeld=%v detHeld=%v, want false/false", slot.vipsHeld, slot.detHeld)
	}
}
