...
// Sentinel живёт в ports/processor (processor.ErrTooManyDetectionConcurrency),
// чтобы app-слой распознавал перегрузку через errors.Is. Здесь — алиас для
// обратной совместимости (используется в shared.NewSemaphore и тестах).
var ErrTooManyDetectionConcurrency = processor.ErrTooManyDetectionConcurrency

// isDetectionOverload распознаёт ошибку ПЕРЕГРУЗКИ detection-семафора
// (переполнение очереди ожидания ИЛИ истечение maxWait — оба случая
// возвращают ErrTooManyDetectionConcurrency из shared.Semaphore.Acquire).
// Используется для graceful degradation: при перегрузке обработка может
// продолжиться с fallback-фокусировкой (center-crop) вместо 503. Ошибки
// самой модели (DetectFaces/DetectObjects) этим предикатом НЕ распознаются —
// они остаются жёсткими ошибками обработки.
func isDetectionOverload(err error) bool {
	if err == nil {
		return false
	}
	return errors.Is(err, ErrTooManyDetectionConcurrency)
}
