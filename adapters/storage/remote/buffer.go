...
type BufferPool struct {
	...
	used atomic.Int64
	max  int64
	// closed — пул закрыт (Close вызван). Guard от повторного Close:
	// счётчик used не сбрасывается повторно, пока живые буферы ещё
	// резервируют память (см. Close).
	closed atomic.Bool
}
...
// Реализует io.Closer.
//
// Guard от повторного Close: если пул уже закрыт, повторный вызов НЕ
// сбрасывает used — живые буферы (ещё не закрытые) продолжают резервировать
// память, и сброс счётчика привёл бы к рассинхронизации бюджета (used <
// фактически занятых байт). Первый Close сбрасывает счётчик; последующие
// вызовы идемпотентны.
