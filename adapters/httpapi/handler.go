...
func (h *Handler) handleAsset(w http.ResponseWriter, r *http.Request) {
	...
	// Генерация выполняется на detached-контексте: жизненный цикл генерации
	// ассета НЕ привязан к HTTP-соединению. context.WithoutCancel сохраняет
	// значения r.Context() (request ID, метаданные для observability), но не
	// отменяется при разрыве соединения клиентом — генерация доживает до
	// конца, и результат попадает в кэш/хранилище. Поверх — собственный
	// бюджет GenerateTimeout (внутренний, не связанный с соединением).
	//
	// Отдача результата клиенту (serveResult) по-прежнему использует
	// r.Context()/writer: разрыв соединения при отдаче тела — нормально.
	genCtx := context.WithoutCancel(r.Context())
	if h.cfg.GenerateTimeout > 0 {
		var cancel context.CancelFunc
		genCtx, cancel = context.WithTimeout(genCtx, h.cfg.GenerateTimeout)
		defer cancel()
	}
...
func (h *Handler) mapError(w http.ResponseWriter, r *http.Request, err error, rawPath string) {
	...
	case generation.OutcomeOverloaded:
		...
		w.Header().Set("Retry-After", strconv.FormatInt(int64(h.cfg.RetryAfter.Seconds()), 10))
...
func (h *Handler) serveFallbackFile(w http.ResponseWriter, r *http.Request, file string) {
	...
	// Буферизованное копирование: буфер берём из copyPool (64 KiB), чтобы не
	// аллоцировать новый на каждый запрос (как в serveSourceObject).
	buf := h.copyPool.Get().([]byte)
	defer h.copyPool.Put(buf)
...
// PathPart извлекает путь из escaped path (до query). Публичная: используется
// admission bypass-check (imager.go) для вычисления asset-пути из URL.
