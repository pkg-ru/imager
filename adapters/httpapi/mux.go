...
//
// bypassCheck — опциональный предикат (nil = выключен): вызывается при
// переполнении admission-семафора. Если возвращает true, запрос
// пропускается в обход семафора — используется для запросов, которые могут
// присоединиться к уже идущей singleflight-генерации того же ассета
// (см. generatev2.Service.InFlightByPath).
func NewMuxWithAdmission(h http.Handler, health *Health, metrics observability.Metrics, auth MetricsAuthConfig, maxConcurrent int, admin http.Handler, bypassCheck func(*http.Request) bool) http.Handler {
	...
	if maxConcurrent > 0 {
		assetHandler = NewAdmissionControl(maxConcurrent).Wrap(h, bypassCheck)
	}
	...
	//
	// Порядок слоёв (снаружи внутрь):
	//   Recover → observability → gzip → mux
	//
	// Recover — САМЫЙ ВНЕШНИЙ слой: паника в ЛЮБОЙ ветке mux (admin, health,
	// metrics, static, asset, gzip) перехватывается и превращается в HTTP 500
	// вместо обрыва соединения. Asset-хендлер имеет собственный recover —
	// этот middleware служит страховкой для остальных веток.
	//
	// Observability middleware находится ВНУТРИ Recover: его defer
	// (DecHttpInflight, завершение gzip-потока) выполняется при панике,
	// потому что паника из внутреннего слоя проходит через defer всех
	// внешних слоёв. Так request ID и inflight gauge продолжают корректно
	// считаться даже при панике.
	inner := observability.NewMiddleware(metrics, gzipHandler(nil, mux))
	return NewRecover(nil, inner)
}
