...
// TestHandlerOverloadedConfigurableRetryAfter проверяет, что Retry-After при
// перегрузке процессора конфигурируем (http.retry-after), а не зашит в "1".
func TestHandlerOverloadedConfigurableRetryAfter(t *testing.T) {
	gen := newFakeGenerator()
	gen.setFallback(&generatev2.OutcomeError{Kind: generatev2.OutcomeOverloaded, Reason: "overloaded"})
	cfg := baseConfig()
	...
	if ra := rec.Header().Get("Retry-After"); ra != "5" {
		t.Fatalf("Retry-After = %q, want 5 (from config)", ra)
	}
	assertErrorCode(t, rec, "overloaded")
}
...
// TestHandlerGenerateDetachedFromClientCancel — жизненный цикл генерации НЕ
// привязан к HTTP-соединению: если r.Context() отменяется во время Generate
// (клиент разорвал соединение), контекст, переданный в сервис, НЕ отменяется,
// и генерация доживает до успешного завершения.
func TestHandlerGenerateDetachedFromClientCancel(t *testing.T) {
	gen := newFakeGenerator()
	gen.addResult("img-png/thumb.png", []byte("PNGDATA"), 7)
	// Блокируем Generate до тех пор, пока тест не отменит r.Context().
	gen.block = make(chan struct{})
	gen.started = make(chan struct{})
	h := newTestHandler(t, gen, baseConfig())

	reqCtx, cancelReq := context.WithCancel(context.Background())
	defer cancelReq()
	req := httptest.NewRequestWithContext(reqCtx, http.MethodGet, "/img-png/thumb.png", nil)
	rec := httptest.NewRecorder()
	...
	// Дожидаемся входа в Generate (контекст уже передан в сервис).
	select {
	case <-gen.started:
	...
	// Клиент разрывает соединение: r.Context() отменяется.
	cancelReq()

	// Контекст, переданный в сервис, НЕ должен быть отменён (detached).
	gen.mu.Lock()
	genCtx := gen.gotCtx
	gen.mu.Unlock()
	if genCtx == nil {
		t.Fatal("Generate was not called with a context")
	}
	if err := genCtx.Err(); err != nil {
		t.Fatalf("context passed to Generate is canceled after client disconnect: %v", err)
	}

	// Разблокируем генерацию: она должна успешно завершиться, несмотря на
	// отмену r.Context().
	close(gen.block)
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, want 200 (generation completed despite client cancel)", rec.Code)
	}
	if body := rec.Body.String(); body != "PNGDATA" {
		t.Errorf("body = %q, want PNGDATA", body)
	}
