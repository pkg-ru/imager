...
type Config struct {
	...
	// перегрузке процессора (OutcomeOverloaded). 0 → дефолт 1s.
...
// DefaultRetryAfter — значение Retry-After по умолчанию (1s) для HTTP 503
// при перегрузке процессора (OutcomeOverloaded).
...
func (c *Config) Validate() error {
	...
	if c.RetryAfter < 0 {
		return fmt.Errorf("httpapi: retry-after must be non-negative")
	}
...
var defaultTimeouts = struct {
	...
	Read:       15 * time.Second,
	// Write: 60s = DefaultGenerateTimeout (30s) + DefaultWriteTimeoutMargin (30s).
	// WriteTimeout должен быть больше GenerateTimeout с запасом на
	// передачу тела, иначе ответ, сгенерированный впритык к таймауту, будет
	// обрезан при записи клиенту (без кода ошибки).
	Write:    60 * time.Second,
	Idle:     60 * time.Second,
	Shutdown: 15 * time.Second,
}

// DefaultWriteTimeoutMargin — запас WriteTimeout над GenerateTimeout.
// Генерация, уложившаяся в GenerateTimeout, должна успеть доехать до клиента:
// WriteTimeout срабатывает на запись тела, и если он <= GenerateTimeout,
// ответ, сгенерированный впритык, будет обрезан без кода ошибки.
const DefaultWriteTimeoutMargin = 30 * time.Second

// DefaultWriteTimeoutFor возвращает дефолтный WriteTimeout для заданного
// GenerateTimeout: GenerateTimeout + запас на передачу тела.
func DefaultWriteTimeoutFor(generateTimeout time.Duration) time.Duration {
	return generateTimeout + DefaultWriteTimeoutMargin
}

// WriteTimeoutTooSmall возвращает true, если явно заданный WriteTimeout
// не оставляет времени на передачу тела после генерации (<= GenerateTimeout).
// 0 (не задан) не считается ошибкой — runtime применит дефолт.
func WriteTimeoutTooSmall(writeTimeout, generateTimeout time.Duration) bool {
	return writeTimeout > 0 && writeTimeout <= generateTimeout
}
