...
// TestCanonicalIDFormatCaseInsensitive проверяет, что регистр ФОРМАТОВ
// (source и output) не влияет на канонический ключ: /a.PNG и /a.png — один
// и тот же ассет. Регистр source_name/segment_name при этом сохраняется.
func TestCanonicalIDFormatCaseInsensitive(t *testing.T) {
	req1, err := Parse("/photos/photo-1-JPG/banner@2.PNG")
	if err != nil {
		t.Fatalf("Parse(upper) error: %v", err)
	}
	req2, err := Parse("/photos/photo-1-jpg/banner@2.png")
	if err != nil {
		t.Fatalf("Parse(lower) error: %v", err)
	}
	id1, err := req1.CanonicalID()
	if err != nil {
		t.Fatalf("CanonicalID(upper) error: %v", err)
	}
	id2, err := req2.CanonicalID()
	if err != nil {
		t.Fatalf("CanonicalID(lower) error: %v", err)
	}
	if !id1.Equal(id2) {
		t.Errorf("formats with different case must canonicalize to the same id: %q != %q", id1.URL(), id2.URL())
	}
	// Регистр source_name сохраняется (семантически значим).
	req3, err := Parse("/photos/Photo-1-jpg/banner@2.png")
	if err != nil {
		t.Fatalf("Parse(case-name) error: %v", err)
	}
	id3, err := req3.CanonicalID()
	if err != nil {
		t.Fatalf("CanonicalID(case-name) error: %v", err)
	}
	if id1.Equal(id3) {
		t.Error("source name case must be preserved (semantically significant)")
	}
}
