...
// не выводится.
//
// Регистр ФОРМАТОВ (source и output) нормализуется к нижнему: расширения
// файлов изображений регистронезависимы, и один и тот же ассет не должен
// создавать разные cache-ключи из-за регистра расширения (/a.PNG vs /a.png).
// Регистр source_name и segment_name НЕ меняется — он семантически значим
// (см. docs/PROCESSING.md, раздел «Регистрозависимость»).
func (r *Request) Build() (string, error) {
	...
	if r.segmentName != "" {
		...
		core = r.sourceName.String() + "-" + strings.ToLower(r.sourceFormat.String()) + "/" + r.segmentName.String()
		...
		core = r.sourceName.String() + "-" + strings.ToLower(r.sourceFormat.String()) + "/" + r.size.String()
		if !r.dpr.IsDefault() {
			core += "@" + strconv.Itoa(r.dpr.Int())
		}
	}
	file := core + "." + strings.ToLower(r.outputFormat.String())
	if r.path == "" {
		return file, nil
	}
