...
// TestRecordSelfDetectionsSurvivesCanceledParent — при отменённом родительском
// ctx боксы self-detection ВСЁ РАВНО сохраняются: запись выполняется на
// detached-контексте (context.WithoutCancel), поэтому разрыв клиента не
// прерывает побочный эффект (иначе следующий запрос повторно вызовет модель).
func TestRecordSelfDetectionsSurvivesCanceledParent(t *testing.T) {
	metaS := newFakeMetadataStore()
	det := newFakeDetector()
	s := newMetaService(metaS, det)
	boxes := []filemeta.PixelBox{{X: 7, Y: 8, Width: 30, Height: 30}}
	...
	// Родительский ctx отменён (разрыв клиента / истёкший GenerateTimeout).
	parentCtx, cancelParent := context.WithCancel(context.Background())
	cancelParent()

	s.recordSelfDetections(parentCtx, object.ObjectKey("src"), fp, processing.OpFaceCrop, boxes, nil)

	metaS.mu.Lock()
	saved := metaS.data["src"]
	metaS.mu.Unlock()
	if saved == nil {
		t.Fatal("sidecar not saved despite canceled parent ctx")
	}
	if len(saved.Faces) != 1 || saved.Faces[0].X != 7 || saved.Faces[0].Confidence != 1.0 {
		t.Fatalf("faces = %+v, want 1 box (7,8,30,30) confidence 1.0", saved.Faces)
	}
...
// TestMetaGateBoundsConcurrency проверяет, что gate метаданных ограничивает
// число одновременно выполняющихся fire-and-forget записей: при заполненном
// gate'е (все metaGateCapacity слотов заняты) новые записи НЕ создают
// горутины (пропускаются с логом), а после освобождения слотов записи снова
// выполняются. Проверяется косвенно через канал-семафор: число занятых
// слотов никогда не превышает ёмкость gate'а.
func TestMetaGateBoundsConcurrency(t *testing.T) {
	metaS := newFakeMetadataStore()
	det := newFakeDetector()
	s := newMetaService(metaS, det)
	// Инициализируем gate (как в New).
	s.metaGate = make(chan struct{}, metaGateCapacity)

	// Заполняем все слоты gate'а (имитация выполняющихся фоновых записей).
	for i := 0; i < metaGateCapacity; i++ {
		if !s.metaGateAcquire() {
			t.Fatalf("metaGateAcquire(%d) failed, want success (slots free)", i)
		}
	}
	// Gate полон: следующая запись должна быть ПРОПУЩЕНА (не создавать
	// горутину), а не ждать слот.
	if s.metaGateAcquire() {
		t.Fatal("metaGateAcquire() succeeded on full gate, want skip")
	}

	// Освобождаем один слот — запись снова возможна.
	s.metaGateRelease()
	if !s.metaGateAcquire() {
		t.Fatal("metaGateAcquire() failed after release, want success")
	}
	// Освобождаем все занятые слоты (включая только что занятый).
	for i := 0; i < metaGateCapacity; i++ {
		s.metaGateRelease()
	}
	// Gate пуст: все слоты снова доступны.
	for i := 0; i < metaGateCapacity; i++ {
		if !s.metaGateAcquire() {
			t.Fatalf("metaGateAcquire(%d) failed after drain, want success", i)
		}
	}
	for i := 0; i < metaGateCapacity; i++ {
		s.metaGateRelease()
