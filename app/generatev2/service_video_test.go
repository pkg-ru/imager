...
// TestVideoExtractSurvivesCanceledParent — при отменённом родительском ctx
// извлечение кадра ВСЁ РАВНО завершается успешно: Extract выполняется на
// detached-контексте (context.WithoutCancel), поэтому разрыв клиента не
// прерывает ffmpeg-работу (иначе кадр теряется и следующий запрос повторяет
// дорогое извлечение).
func TestVideoExtractSurvivesCanceledParent(t *testing.T) {
	ext := newFakeVideoExtractor()
	metaS := newFakeMetadataStore()
	env := videoEnv(t, ext, metaS)
	env.src.Add("clip.mp4", []byte("VIDEO-BYTES"))
	...
	res, err := env.svc.extractVideoFrame(parentCtx, object.ObjectKey("clip.mp4"))
	if err != nil {
		t.Fatalf("extractVideoFrame with canceled parent ctx: %v", err)
	}
	if res == nil || len(res.Frame) == 0 {
		t.Fatal("extractVideoFrame returned empty frame")
	}

	// VideoExtractor вызван ровно один раз и получил ВАЛИДНЫЙ (не отменённый)
	// detached-контекст.
	if ext.callCount() != 1 {
		t.Fatalf("VideoExtractor calls = %d, want 1", ext.callCount())
	}
	if ext.lastCtxErr() != nil {
		t.Fatalf("VideoExtractor received canceled ctx (err = %v), want valid detached ctx", ext.lastCtxErr())
	}
}
