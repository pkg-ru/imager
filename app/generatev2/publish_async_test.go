...
func (f *fakePubMetrics) IncDetectionDegraded()                                               {}
func (f *fakePubMetrics) IncVipsOrphanOp()                                                    {}
func (f *fakePubMetrics) SetVipsOrphanInflight(int64)                                         {}
func (f *fakePubMetrics) IncPanics()                                                          {}
...
// TestSyncFallbackPublishSurvivesCanceledCtx — sync-fallback публикация
// (Disabled=true / переполнение очереди / shutdown) выполняется на
// detached-контексте: даже если входной ctx отменяется ВО ВРЕМЯ
// sync-публикации (клиент разорвал соединение), результат ВСЁ РАВНО попадает
// в кэш, а не теряется с OutcomeCanceled.
func TestSyncFallbackPublishSurvivesCanceledCtx(t *testing.T) {
	release := make(chan struct{})
	res := newSlowResultStore(release)
	// Disabled=true → публикация всегда синхронная (sync-fallback путь).
	env := newTestEnv(t, func(d *Deps) {
		d.Results = res
		d.PublishQueue = &PublishQueueConfig{Disabled: true}
	})
	...
	keyCh := make(chan object.ObjectKey, 1)
	go func() {
		r, err := env.svc.Generate(cancelCtx, mustReq(t, "", "photo", "png", asset.CropCenter, false, "100x100", 2, "webp"))
		if err == nil {
			keyCh <- r.Key
			_ = r.Close()
		}
	...
	// Дожидаемся входа sync-публикации в Publish (блокируется в slow store).
	res.waitStarted(t)

	// Клиент разрывает соединение: входной ctx отменяется ВО ВРЕМЯ
	// sync-публикации.
	cancel()

	// Sync-fallback НЕ должен прерваться отменой ctx: разблокируем запись,
	// публикация завершается, объект попадает в кэш.
	close(release)
	select {
	case err := <-genDone:
		if err != nil {
			t.Fatalf("Generate: %v, want nil (publish completed despite canceled ctx)", err)
		}
	case <-time.After(3 * time.Second):
		t.Fatal("Generate did not finish after release")
	}
	key := <-keyCh
	if !res.Has(key) {
		t.Fatal("sync fallback did not publish object despite canceled ctx")
	}
}
