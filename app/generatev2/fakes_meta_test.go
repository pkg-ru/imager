...
func (s *fakeMetadataStore) Update(_ context.Context, srcKey string, fn metadata.UpdateFn) error {
	...
	if !ok {
		...
		// Работаем на копии, как реальная реализация (fs.MetadataStore):
		// fn может мутировать вход свободно, не затрагивая объект в store.
		// Иначе асинхронные записи (updateLargestAIAssetAsync и т.п.) гоняли
		// бы с чтениями теста, которые берут указатель из s.data.
		m = m.Clone()
	}
	changed, err := fn(m)
	if err != nil {
