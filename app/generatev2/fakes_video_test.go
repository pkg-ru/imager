...
type fakeVideoExtractor struct {
	...
	// lastCtxErrVal — ctx.Err() на момент вызова Extract (для проверки, что
	// извлечение получает валидный detached-контекст при отменённом родителе).
	lastCtxErrVal error
}
...
func (f *fakeVideoExtractor) Extract(ctx context.Context, _ io.ReadSeeker, opts videoframe.Options) (*videoframe.Result, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.calls++
	f.lastOpts = opts
	f.lastCtxErrVal = ctx.Err()
	if f.err != nil {
		return nil, f.err
	}
...
// lastCtxErr возвращает ctx.Err() на момент последнего вызова Extract.
func (f *fakeVideoExtractor) lastCtxErr() error {
	f.mu.Lock()
	defer f.mu.Unlock()
	return f.lastCtxErrVal
}
