...
// detectionPersistTimeout — собственный таймаут записи боксов self-detection
// в sidecar-метаданные. Запись выполняется на detached-контексте
// (context.WithoutCancel): разрыв клиента не должен прерывать сохранение
// боксов (иначе следующий запрос повторно вызовет модель — нарушение
// инварианта «1 вызов модели на родителя»). Таймаут ограничивает время
// записи в remote (fsync/upload с retry), если родительский ctx уже истёк.
const detectionPersistTimeout = 30 * time.Second
...
func (s *Service) recordSelfDetections(
	...
	// Запись боксов self-detection выполняется на detached-контексте: разрыв
	// клиента не должен прерывать сохранение (иначе следующий запрос повторно
	// вызовет модель). context.WithoutCancel сохраняет значения ctx (request
	// ID, метаданные), но не отменяется при разрыве соединения. Если входной
	// ctx уже отменён/истёк, поверх detached-контекста накладывается
	// собственный таймаут detectionPersistTimeout (ограничение времени записи
	// в remote). Ретраи/backoff внутри Metadata.Update уважают этот
	// detached-контекст, а не клиентский.
	dctx := context.WithoutCancel(ctx)
	if ctx.Err() != nil {
		var dCancel context.CancelFunc
		dctx, dCancel = context.WithTimeout(dctx, detectionPersistTimeout)
		defer dCancel()
	}
	_, err := s.deps.Coordinator.Do(dctx, metaFlightKey(metaKey), func() (any, error) {
		return nil, s.deps.Metadata.Update(dctx, metaKey.String(), func(m *filemeta.FileMetadata) (bool, error) {
			if m == nil {
				m = filemeta.NewFileMetadata()
			}
...
//
// Ограничение конкурентности: число одновременно выполняющихся фоновых
// записей метаданных ограничено gate'ом (metaGate, ёмкость
// metaGateCapacity). При переполнении gate'а запись ПРОПУСКАЕТСЯ (с логом) —
// это best-effort операция, и ожидание слота могло бы накапливать
// неограниченное число горутин при всплеске генераций.
func (s *Service) updateLargestAIAssetAsync(
	...
	if !s.metaGateAcquire() {
		s.log.Warnf("generatev2: metadata gate full, skip largest_ai_asset update (asset=%s)", assetKey)
		return
	}
	go func() {
		defer s.metaGateRelease()
...
func (s *Service) recordAssetCreationTime(ctx context.Context, assetKey object.ObjectKey) {
	...
	if !s.metaGateAcquire() {
		s.log.Warnf("generatev2: metadata gate full, skip created_unix record (asset=%s)", assetKey)
		return
	}
