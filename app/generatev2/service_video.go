...
//
// Извлечение выполняется на detached-контексте: разрыв клиента не должен
// прерывать ffmpeg-работу (иначе кадр теряется и следующий запрос повторяет
// дорогое извлечение). context.WithoutCancel сохраняет значения ctx (request
// ID, метаданные), но не отменяется при разрыве соединения. Если входной ctx
// уже отменён/истёк, поверх detached-контекста накладывается собственный
// таймаут videoExtractTimeout; ffmpeg-процесс (exec.CommandContext) уважает
// этот контекст и корректно убивается при его истечении.
func (s *Service) extractVideoFrame(ctx context.Context, srcKey object.ObjectKey) (*videoframe.Result, error) {
	...
	if ctx.Err() != nil {
		...
		ectx, eCancel = context.WithTimeout(ectx, videoExtractTimeout)
		defer eCancel()
	}
	src, err := s.deps.Sources.Open(ectx, srcKey)
	if err != nil {
		if object.IsNotFound(err) {
			return nil, outcome(OutcomeNotFound, "source not found", err)
		}
		return nil, s.mapSourceError(ectx, err)
	}
	defer src.Close()

	res, err := s.deps.VideoExtractor.Extract(ectx, src, s.videoOptions())
	if err != nil {
