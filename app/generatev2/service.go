...
// клиента на время записи в remote (fsync/upload с retry до 2s).
const (
	...
	// syncPublishTimeout — собственный таймаут синхронной (fallback)
	// публикации результата в кэш. Публикация выполняется на detached-
	// контексте (context.WithoutCancel) и не должна прерываться отменой
	// клиентского контекста; таймаут ограничивает время записи в remote
	// (fsync/upload с retry).
	syncPublishTimeout = 30 * time.Second
	// metaGateCapacity — ёмкость gate-канала, ограничивающего число
	// одновременно выполняющихся fire-and-forget goroutine'ов метаданных
	// (updateLargestAIAssetAsync, recordAssetCreationTime). Эти записи
	// best-effort и не влияют на результат генерации, поэтому при
	// переполнении gate'а запись ПРОПУСКАЕТСЯ (с логом), а не ждёт слота.
	metaGateCapacity = 32
)
...
type Service struct {
	...
	// metaGate — bounded gate для fire-and-forget goroutine'ов метаданных
	// (updateLargestAIAssetAsync, recordAssetCreationTime). Ограничивает
	// число одновременно выполняющихся фоновых записей метаданных, чтобы
	// всплеск генераций не порождал неограниченное число горутин. Каждый
	// слот канала = одна выполняющаяся запись; при переполнении запись
	// пропускается (best-effort, см. metaGateAcquire).
...
func New(d Deps) (*Service, error) {
	...
	// Gate для fire-and-forget goroutine'ов метаданных: bounded-канал
	// ёмкостью metaGateCapacity. Слот канала = одна выполняющаяся запись.
	s.metaGate = make(chan struct{}, metaGateCapacity)
...
// metaGateAcquire пытается занять слот gate'а метаданных. Возвращает true,
// если слот занят (запись можно выполнять в фоне). При переполнении gate'а
// (все слоты заняты) возвращает false — запись ПРОПУСКАЕТСЯ: это best-effort
// операция, не влияющая на результат генерации, и ожидание слота могло бы
// накапливать неограниченное число горутин.
func (s *Service) metaGateAcquire() bool {
	if s.metaGate == nil {
		return true
	}
	select {
	case s.metaGate <- struct{}{}:
		return true
	default:
		return false
	}
...
// metaGateRelease освобождает слот gate'а метаданных. Вызывается в defer
// фоновой записи, парно к успешному metaGateAcquire.
func (s *Service) metaGateRelease() {
	if s.metaGate == nil {
		return
	}
	<-s.metaGate
...
// InFlightByPath сообщает, выполняется ли в данный момент singleflight-
// генерация для ассета, заданного URL-путём (escaped path без query).
// Выполняет полный парс пути и вычисление канонического ключа (как
// Generate), затем проверяет Coordinator.InFlight.
//
// Используется admission control для bypass: когда семафор переполнен,
// запрос, который может присоединиться к уже идущей генерации того же
// ассета, пропускается в обход семафора (вместо 503). Метод дешёвый:
// парс + canonicalize + lookup в map под блокировкой, без аллокаций и без
// обращения к хранилищам/процессору.
//
// Возвращает false, если путь не является валидным asset URL, политика
// запрещает запрос, формат не медиа или генерация для ключа не идёт.
func (s *Service) InFlightByPath(ctx context.Context, path string) bool {
	if s == nil || s.deps.Coordinator == nil {
		return false
	}
	...
	// Policy decision (deny-by-default) — повторяем логику Generate, чтобы
	// ключ совпадал с тем, под которым идёт генерация.
	learning := s.learningEnabled()
	if req.IsPreset() && !req.IsResolved() {
		resolved, dec := s.deps.Policy.Resolve(req)
		if !dec.Allowed {
	...
	key := object.ObjectKey(url)
	return s.deps.Coordinator.InFlight(key)
}
...
func (s *Service) processAndPublish(ctx context.Context, key object.ObjectKey, in processor.Input) (buffer.Buffer, error) {
	...
	if procErr != nil {
		...
		if processor.IsTooManyConcurrency(procErr) {
	...
	//
	// Публикация выполняется на detached-контексте: жизненный цикл записи
	// результата в кэш НЕ привязан к клиентскому ctx. context.WithoutCancel
	// сохраняет значения ctx (request ID, метаданные), но не отменяется при
	// разрыве соединения — результат ВСЁ РАВНО попадает в кэш. Если входной
	// ctx уже отменён/истёк, поверх detached-контекста накладывается
	// собственный таймаут syncPublishTimeout (ограничение времени записи в
	// remote). Ретраи/backoff внутри publishFromBuffer уважают этот
	// detached-контекст, а не клиентский.
	pubCtx := context.WithoutCancel(ctx)
	if ctx.Err() != nil {
		var pubCancel context.CancelFunc
		pubCtx, pubCancel = context.WithTimeout(pubCtx, syncPublishTimeout)
		defer pubCancel()
	}
	pubStart := time.Now()
	pubErr := s.publishFromBuffer(pubCtx, key, reader)
...
//
// ErrWaitTimeout — истёк WaitTimeout ожидания завершения владельца
// singleflight ("зависший" владелец). Это временная недоступность: клиенту
// следует повторить позже (503 + Retry-After), а не считать запрос
// ошибочным. Маппится в OutcomeUnavailable с явной причиной.
func (s *Service) mapCoordinatorError(ctx context.Context, err error) error {
	return mapOutcomeError(ctx, err, func(err error) error {
		...
		if errors.Is(err, singleflight.ErrWaitTimeout) {
			return outcome(OutcomeUnavailable, "coordination wait timeout", err)
		}
