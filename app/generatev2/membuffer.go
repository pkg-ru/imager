...
func (b *memBuffer) releaseLocked() {
	if b.closed && b.refs <= 0 {
		...
		for _, c := range b.chunks {
			// Затираем данные перед возвратом в пул: чанк переиспользуется
			// другим буфером, и старые данные не должны протекать в новый
			// контент. Изображение не является секретом, но zeroing 32KiB
			// чанка дёшев и исключает случайную утечку данных между
			// запросами (например, при частичной записи нового буфера).
			for i := 0; i < len(c); i++ {
				c[i] = 0
			}
			cp := c[:0]
