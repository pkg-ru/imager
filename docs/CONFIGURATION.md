...
Детектор лиц/объектов для операций `fc`/`oc` (face-crop/object-crop) и `ffx`/`ofx` (face-fix/object-fix). Требует сборки с `-tags onnx`. Включение операции задаётся путём к модели: пустой путь = операция отключена.
