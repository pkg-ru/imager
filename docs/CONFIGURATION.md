...
1. копирует отсутствующие базовые конфиги и шаблоны из `/etc/imager` (дефолты образа) в каталог `IMAGER_CONFIG_DIR` — **только если целевой файл ещё не существует**;
2. для каждого шаблона `*-local.yaml.example` в `IMAGER_CONFIG_DIR` создаёт соответствующий `*-local.yaml`, также **только если целевой файл ещё не существует**.

Существующие (пользовательские или volume-смонтированные) конфиги никогда не перезаписываются. Это позволяет:
...
- переопределять **все конфиги целиком** — смонтировать свою папку с полным набором `server.yaml`/`generate.yaml`/`failback.yaml` (+ `*-local.yaml`).
