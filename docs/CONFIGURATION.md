...
Кадровый семафор (`libvips.frame-workers.*`) — отдельный bounded-семафор для параллельной покадровой обработки анимаций (GIF/APNG/WebP/AVIF/JXL) в `withFrames`: создание кадров идёт последовательно, а тяжёлые покадровые операции (watermark/crop/resize+embed/trim/flip/detection-crop) выполняются пулом воркеров на готовых кадрах. Семафор НЕ зависит от основного libvips-лимита конкурентности (`libvips.limits.concurrency`): он ограничивает параллелизм ВНУТРИ одного запроса, тогда как основной — между запросами (иначе 8 запросов × 8 кадров дали бы 64 одновременных vips-операции).

| Ключ (`libvips.frame-workers.*`) | Тип | По умолчанию | Описание |
|:------------------------------------|-----|--------------|----------|
| `workers` | int | `min(GOMAXPROCS, 4)` | Максимум одновременных покадровых операций внутри одного запроса (`0` = дефолт; клэмп до [1, 8]; отрицательное — ошибка старта) |
