...
Применимы к s3/sftp/ftp/ftps/http в обеих секциях. Колонка «Применимо» уточняет,
какие ключи действуют для каких типов: пул idle-соединений и кэш метаданных
относятся только к HTTP-подобным хранилищам (S3/HTTP), а `max-conns` — к
SFTP/FTP/FTPS.

| Ключ | По умолчанию | Применимо | Описание |
|------|--------------|-----------|----------|
| `spool-dir` | системный tmp | все | Каталог временных spool-файлов при чтении remote-объектов |
| `spool-max-bytes` | `0` (нет) | все | Лимит размера spool; превышение → ошибка квоты |
| `dial-timeout` | `"30s"` | все | Таймаут установления соединения |
| `read-timeout` | `"60s"` | S3/HTTP/SFTP/FTP/FTPS | Таймаут выполнения операции (серия попыток) |
| `max-attempts` | `3` | S3/HTTP/SFTP/FTP/FTPS | Число попыток операции |
| `max-conns` | `2` | SFTP/FTP/FTPS | Максимум одновременных соединений в пуле |
| `max-idle-conns` | `100` | S3/HTTP | Максимум idle-соединений в пуле |
| `max-idle-conns-per-host` | `10` | S3/HTTP | Максимум idle-соединений на хост |
| `idle-conn-timeout` | `"90s"` | S3/HTTP | Время жизни idle-соединения |
| `metadata-ttl` | `"30s"` | S3 | TTL кэша метаданных S3 (`0` = кэш отключён) |
...
Для `fs`-результатов janitor запускается автоматически при старте сервиса и останавливается при graceful shutdown.
