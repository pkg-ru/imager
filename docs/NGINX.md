...
Наиболее быстрый вариант при наличии локального SSD/NVMe.
...
Подходит, если готовые объекты публичны (примеры провайдеров — в разделе «Режим: публичный S3»).
...
Nginx не должен самостоятельно получать приватные S3-объекты: доступом владеет `imager`, nginx только проксирует запрос. `proxy_cache` включается опционально (см. раздел «Proxy cache для S3»).
...
В текущем imager публичными являются asset URL, `/healthz` и `/readyz`; `/metrics` и `/admin/*` являются служебными эндпоинтами.
...
`try_files` сначала проверяет result, затем source и только после этого делает внутренний переход в `@imager`.
...
Особенно удобно для immutable assets: после первой генерации последующие запросы не доходят до imager.

Эндпоинт и способ формирования публичного URL зависят от S3-провайдера: Selectel поддерживает public bucket и отдельные публичные домены объектов, Yandex Object Storage позволяет отдельно разрешать анонимное чтение объектов.

---

## 6. Режим: приватный S3

Если result хранится в приватном S3, основной эндпоинт nginx должен проксировать запрос в imager:
...
Наиболее безопасный вариант: S3 credentials не появляются в nginx.
...
Для immutable asset URL query string обычно не является частью идентичности объекта, поэтому вместо стандартного ключа nginx (близкого к `$scheme$proxy_host$request_uri`, т.е. включающего `$request_uri`) можно использовать:
...
Не следует пытаться сделать HTTP-ответ nginx и прямой ответ imager побайтно идентичными по служебным заголовкам: при статической отдаче nginx самостоятельно формирует `ETag`, `Last-Modified` и поддерживает range-запросы, а при отдаче через imager действуют его собственные правила.

Для immutable assets это не проблема:
...
Усложнять конфигурацию ради искусственного совпадения `ETag` не нужно.

---

## 12. Buffering

Для imager/S3 `proxy_buffering on;` оставляется включённым: nginx забирает ответ от upstream максимально быстро и при необходимости использует временный файл на диске, не удерживая соединение из-за медленного клиента.

Без измерений не задавайте `proxy_buffers 8 16k; proxy_buffer_size 16k;` как обязательные параметры — стандартные значения nginx являются нормальной стартовой точкой.
...
Не следует без причины использовать все механизмы одновременно (`FS` + `try_files` + `proxy_cache` + `S3 cache` + `imager cache`). Цепочка выбирается согласно origin:
...
- **Быстрый локальный SSD** — `result/source = FS`, nginx с `try_files`, `proxy_cache = OFF`. Самый простой и быстрый вариант.
- **Public S3** — `nginx → public S3`, при 404 fallback в imager; `proxy_cache` опционально. Object storage может иметь собственный edge/cache-механизм (например, Selectel кэширует объекты public bucket и учитывает `Cache-Control`).
- **Private S3** — `nginx → imager → private S3`, при необходимости с `proxy_cache` перед imager. Наиболее универсальный production-вариант для приватного объектного хранилища.
...
Общие proxy-заголовки и таймауты должны находиться в одном snippet, чтобы не копироваться по нескольким `location`.

Отдельные конфигурации для FS/S3 не нужны — отличается только механизм получения существующего объекта:
...
Всё остальное — TLS, HTTP/2, health-эндпоинты, security, proxy headers, таймауты, buffering и опциональный cache — задаётся один раз.
