...
`proxy_cache` не используется. Наиболее быстрый вариант при наличии локального SSD/NVMe.
...
Для immutable assets после первой генерации последующие запросы не доходят до imager.
...
S3 credentials не появляются в nginx.
...
Для imager/S3 `proxy_buffering on;` включён: nginx забирает ответ от upstream быстро, при необходимости используя временный файл на диске.
...
Эти ограничения настраиваются под реальную инфраструктуру; в базовый конфиг не включаются.

---

## 15. Что не нужно делать

Не используйте без причины все механизмы одновременно (`FS` + `try_files` + `proxy_cache` + `S3 cache` + `imager cache`). Цепочка выбирается согласно origin:
