...
Для локального FS `proxy_cache` не нужен: ОС уже кэширует прочитанные страницы файлов, а дополнительный слой хранения только усложняет конфигурацию и расходует место.
...
При статической отдаче nginx сам формирует `ETag`/`Last-Modified` и поддерживает range-запросы; при отдаче через imager действуют его собственные правила. Для immutable assets совпадение служебных заголовков не требуется.

---

## 12. Buffering

Для imager/S3 `proxy_buffering on;` оставлен включённым: nginx забирает ответ от upstream быстро, при необходимости используя временный файл на диске. Стандартные значения буферов — нормальная стартовая точка.
