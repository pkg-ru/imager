...
### Face-fix (`crop: face_fix`) и Object-fix (`crop: object_fix`)
...
|--------|----------|------------|
| YuNet (`face-model`) | `fc`/`fct`, `ffx`/`ffxt` | Детекция лиц |
| SSD/YOLO-подобная (`object-model`) | `oc`/`oct`, `ofx`/`ofxt` | Детекция объектов |
