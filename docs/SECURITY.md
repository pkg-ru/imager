...
| Admission control | `http.max-concurrent-requests` (или fallback из `application.limits.concurrency × 4`, если не задан) → `503` + динамический `Retry-After`; health/metrics остаются доступными |
