...
Всё запрещено по умолчанию; разрешается только явно покрытое правилами. Реализация — `internal/domain/policy` (компиляция конфигурации в неизменяемую политику на старте, fail-fast при невалидных правилах).
...
Парсер asset URL (`internal/domain/asset`) отклоняет:
