...
func NewServer(cfgDir string, opts ...Option) (*Server, error) {
	...
	app, err := composition.Build(context.Background(), composition.AppOptions{
		...
		SingleflightWaitTimeout: rc.SingleflightWaitTimeout,
	...
	//
	// WriteTimeout должен быть больше GenerateTimeout с запасом на
	// передачу тела. Если write-timeout не задан явно, вычисляем дефолт как
	// GenerateTimeout + запас; если задан явно и <= GenerateTimeout — логируем
	// предупреждение о риске обрезанных ответов (значение сохраняем: явная
	// конфигурация имеет приоритет).
	writeTimeout := rc.Server.WriteTimeout
	if writeTimeout <= 0 {
		writeTimeout = httpapi.DefaultWriteTimeoutFor(rc.HTTP.GenerateTimeout)
	}
	if httpapi.WriteTimeoutTooSmall(writeTimeout, rc.HTTP.GenerateTimeout) {
		logger.Warnf("server.write-timeout (%s) <= http.generate-timeout (%s): "+
			"ответ, сгенерированный впритык к таймауту, может быть обрезан "+
			"при записи клиенту; увеличьте write-timeout", writeTimeout, rc.HTTP.GenerateTimeout)
	}
	...
	// asset-запросов лимитом из конфигурации. Приоритет: явный
	// http.max-concurrent-requests; если он не задан (0), используется
	// application.limits.concurrency (со множителем) как fallback — см.
	// composition.EffectiveAdmissionLimit. Если оба не заданы — 0
	// (admission выключен).
	admissionMax := composition.EffectiveAdmissionLimit(rc.HTTP.MaxConcurrentRequests, rc.Limits.Concurrency)
	// Bypass admission: при переполнении семафора запрос, который может
	// присоединиться к уже идущей singleflight-генерации того же ассета,
	// пропускается в обход семафора (вместо 503). Колбэк дешёвый: парс пути
	// + canonicalize + lookup в singleflight под блокировкой. nil, если
	// admission выключен (maxConcurrent == 0) — колбэк не вызывается.
	var bypassCheck func(*http.Request) bool
	if admissionMax > 0 && app.Service != nil {
		bypassCheck = func(r *http.Request) bool {
			return app.Service.InFlightByPath(context.Background(), httpapi.PathPart(r.URL.EscapedPath()))
		}
	}
	handler := httpapi.NewMuxWithAdmission(app.Handler, health, metrics,
		rc.Server.MetricsAuth, admissionMax, app.AdminHandler, bypassCheck)
